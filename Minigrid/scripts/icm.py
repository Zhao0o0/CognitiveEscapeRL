...
class ICM:
    def __init__(
        ...
        preprocess_obss,
        reward_weighting,
        uncertainty_budget=2.5,  #0.5  #1     #0.1,  # 0.05,
        mu=0.5,   #1     #0.5
        grad_clip=40,
    ):
    ...
    def compute_intrinsic_rewards(self, old_obs, new_obs, action):
        ...
        ##    self.device
        ##)
        action_vector = torch.stack([action] * 109).clone().detach().float().to(self.device)
        
        action_vector /= 6
        forward_prediction, uncertainty = self.autoencoder(old_obs, action_vector)
        #print('icm:old_obs:{},action_vector:{}'.format(old_obs,action_vector))
        ...
        if self.uncertainty == "True":
            ...
            reward = mse - self.mu * torch.exp(uncertainty)
        ...
        #print('loss:{},mse:{},uncertainty:{},reward:{}'.format(loss,mse,uncertainty,reward))

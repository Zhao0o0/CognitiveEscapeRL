...
class A2CAlgo(BaseAlgo):
    ...
    def __init__(
        ...
        self.visitation_record_map_list = [np.zeros(
            (self.env.envs[0].width, self.env.envs[0].height)
        )]*16
    ...
    def collect_experiences(self):
        ...
        for i in range(self.num_frames_per_proc):
            ...
            if self.random_action == "True":
                action = np.random.randint(6, size=16)    #列表长度16
            elif self.random_action == "False":
                action = action_agent.cpu().numpy()
            else:
            ...
            #判断是否局部最优
            '''
            for i_env in range(len(self.env)):
                #执行逃逸策略,对列表的动作改变
                if self.local_optimum[i_env]:
                    print('before: i_env:{},action[i_env]:{}'.format(i_env,action[i_env]))
                    action_escape = np.random.randint(3)
                    action[i_env] = action_escape
                    print('after: i_env:{},action[i_env]:{}'.format(i_env,action[i_env]))
            ...
            '''
            obs, extrinsic_reward, terminated, truncated, info = self.env.step(action)     #这样的话事实上self.random_action没有起作用
            #print('terminated:{}',terminated)
            print('truncated:{}',truncated)
            #done = terminated or truncated
            done = tuple(value1 or value2 for value1, value2 in zip(terminated, truncated))            
            #print('action',action)            
            #print('extrinsic_reward',extrinsic_reward)
            print('done',done)
            ...
            self.num_visited_positions_history.append(self.num_visited_position_list)    #是队列，所以可以用append，一直保持队列长度固定
            ...
            if len(self.num_visited_positions_history) >= self.max_deque_length:
                ...
                for first_list_i in range(len(self.env)):
                    ...
                    if num_difference < 1:         #表示一直保持在一个位置
